(function(window, document) {
    ...
    var SpeechSynthesisUtterancePolyfill = function(text) {
        ...
        /** Added by Arvind **/
        /** Support Cordova Media API **/
        this._mediaPlayback = function() {
            ...
            function _play(audioURL) {
                var media = new Media(audioURL,
                    // success callback
                    function() {
                        console.log("playAudio():Audio Success");
                        // Check for any pending sentences and play
                        if (sentences.length) {
                            _play(getAudioUrl(that.corsProxyServer, sentences.shift(), that.lang));
                        } else {
                            that._ended = true;
                        }
                    },
        ...
        var playNext = function(utteranceQueue) {
            ...
            if (SpeechSynthesisUtterancePolyfill) {
                ...
                if (typeof Media != "undefined") {
                    // Hybrid
                    SpeechSynthesisUtterancePolyfill._mediaPlayback();
                } else {
